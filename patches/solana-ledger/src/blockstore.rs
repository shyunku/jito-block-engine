...
// === BEGIN BE PATCH =================================================
impl Blockstore {
    /// 반환값을 통해 BE 쪽에서 `try_catch_up_with_primary()` 를 호출한다.
    /// Solana 2.3.6 기준으로 `Rocks` 의 메모리 레이아웃 첫 필드는
    /// rocksdb::DB 이므로 간단한 포인터 캐스팅으로 &DB 를 얻는다.
    pub fn rocks(&self) -> &rocksdb::DB {
        // Arc<Rocks> → &Rocks → &rocksdb::DB  (unsafe 캐스팅)
        unsafe { &*(&*self.db as *const _ as *const rocksdb::DB) }
    }
}
// === END BE PATCH ===================================================
